...
export const getAllBranches = () => async (dispatch: AppDispatch): Promise<boolean> => {
  try {
...
export const createBranch = (formData: FormData) => async (): Promise<boolean> => {
  const toastId = toast.loading("Creating...")
  try {
    const res = await apiConnector("POST", BranchEndpoints.CREATE_BRANCH_API, formData);
