"use client"
import { Home, UserRound, ChevronRight, LogOut, Settings, Menu, X } from "lucide-react";
...
const Sidebar = ({ children }: { children: React.ReactNode }) => {
  ...
  const navItems = [
    ...
    { href: "/dashboard/staff-list", label: "Staff List", icon: <UserRound className="h-5 w-5" /> },
  ];
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-60 lg:flex-col fixed h-full">
        <div className="flex flex-col flex-grow bg-primary pt-5 pb-4 overflow-y-auto">
          ...
          <div className="flex-shrink-0 flex border-t border-white/20 p-4">
            <button className="flex items-center w-full px-4 py-2 text-sm font-medium text-white/90 hover:bg-white/10 rounded-lg transition-colors">
      ...
      <div className="lg:pl-60 flex flex-col flex-1">
        {/* Navbar */}
        <header className={`sticky top-0 z-40 transition-all duration-300 ${isScrolled ? "bg-primary shadow-md" : "bg-primary"}`}>
          ...
          <div className="px-4 sm:px-6 lg:px-8">
            ...
            {pathname !== "/dashboard" && (
              <div className="breadcrumbs text-sm mb-6 mt-4">
                <ul>
                  <li>
                    <Link href="/dashboard" className="text-primary hover:text-primary-focus font-medium">
                      Dashboard
                    </Link>
                  </li>
                  {parts.map((item, index) => {
                    ...
                    return (
                      <li key={item}>
                        {isLast ? (
                          <span className="capitalize font-semibold text-gray-600">{item}</span>
                        ) : (
                          <Link className="capitalize font-medium text-primary hover:text-primary-focus" href={href}>
                            {item}
                          </Link>
                        )}
                      </li>
      ...
      <div className={`lg:hidden fixed inset-0 z-50 transition-opacity duration-300 ${isOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
        ...
        <div className={`fixed inset-y-0 left-0 max-w-xs w-full bg-primary shadow-xl transform transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
